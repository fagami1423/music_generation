...
from .helper_utils import save_midi, convert_midi_files_in_folder, convert_midi_to_wav
...
class RNNModel:
    ...
    @classmethod
    def _load_model(cls, config_id: str):
        if config_id not in cls._model_cache:
            bundle_name = f"{config_id}.mag"
            bundle = sequence_generator_bundle.read_bundle_file(os.path.join("models/melody/bundles", bundle_name))
            generator_map = performance_sequence_generator.get_generator_map()
            generator = generator_map[config_id](
                checkpoint=None,
                bundle=bundle
            )
            cls._model_cache[config_id] = generator

        return cls._model_cache[config_id]

    def generate(self,bundle_name: str,
             sequence_generator,
             generator_id: str,
            ...
            generator = self._load_model(self.config_id)
            ...
            if primer_filename:
              primer_sequence = mm.midi_io.midi_file_to_note_sequence(
                os.path.join("models/melody/primers", primer_filename))
            else:
            ...
            save_midi(sequence, "mixer","mixer", primer_filename.split(".")[0])
